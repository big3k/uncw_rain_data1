...
nxd = 100
storm = read_mat.get_storm()  # read in a storm case, 100x100   
true_object_np = np.copy(storm) 
...
axs1[0, 2].imshow(true_object_np, cmap='Greys_r', vmin=0, vmax=20)
...
def np_to_torch(np_array):	return torch.from_numpy(np_array.copy()).float() 

true_object_torch = np_to_torch(true_object_np).to(device)
storm_torch = np_to_torch(storm).to(device)

# noisy example
measured_data = torch.poisson(true_object_torch)
# add gaps 
mask_image = torch.ones_like(measured_data)
mask_image[int(0.55*nxd):int(0.75*nxd), int(0.55*nxd):int(1.00*nxd)] = 0
#mask_image[int(0.15*nxd):int(0.25*nxd), int(0.15*nxd):int(0.35*nxd)] = 0

#measured_data = measured_data * mask_image
measured_data = storm_torch * mask_image # no noise for now

axs1[0, 2].imshow(torch_to_np(true_object_torch), cmap='Greys_r', vmin=0, vmax=20)
axs1[0, 2].set_title('TRUE') 
axs1[0, 2].set_axis_off() 

axs1[0, 1].imshow(torch_to_np(measured_data), cmap='Greys_r', vmin=0, vmax=20)
...
for ep in range(10000 +1): 
	...
	if ep % 2 == 0:
		...
		cv2.waitKey(1)
		axs1[1, 2].cla()
		axs1[1, 2].imshow(torch_to_np(best_recon), cmap='Greys_r', vmin=0, vmax=20) 
		axs1[1, 2].set_axis_off() 
		axs1[1, 1].cla() 
		axs1[1, 1].imshow(torch_to_np(output_image), cmap='Greys_r', vmin=0, vmax=20) 
