...
# 4/11/2026: 
# v3: GFS changed from 1-deg to 0.5-deg since Aug. 11, 2025.
#     Change 5-day per order to 2-day per order, as the files are bigger, and there is a 250GB-per-order 
#      limit. 
...
# ------------------ config params
wait_sec=120  # in seconds. Intervals to check if files are staged, also time to wait all the files are ready. 
              # for GFS 3, 1-deg, 30 sec. shall be enough. But GFS 4, 0.5-deg, it needs more time. 
#wait_limit=3600 # total seconds to wait before downloading whatever is given, b/c there are missing files. 
wait_limit=600 # total seconds to wait before downloading whatever is given, b/c there are missing files. 
...
# Yalei -- change below to suit your machine:
#tar_dir=/data/snow/yyou/GFS_auto_download/raw_tars # dir to save the downloaded tar files.
#unpack_dir=/data/snow/yyou/GFS_auto_download/GFS_00-03 # dir to contain unpacked/weeded files
#html=/data/snow/yyou/GFS_auto_download/html  #dir to store the responses from the web server
...
#===================== End of Grand Loop of 2-Day Chunks ============================
echo "Done" 
exit
