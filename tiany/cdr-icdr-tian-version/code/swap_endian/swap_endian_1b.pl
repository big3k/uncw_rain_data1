...
#  02 Nov 2024 YDT     Use the -d option for output directory. -f contains full input
#                      file paths
#####################################################################################
...
if (!$opts{s} or !$opts{f} or !$opts{d}) 
	{ die "\n\nCorrect usage:\n  $PROGRAM_NAME -s Structure_file -f File-list_file -d output_directory_path\n"; }
...
foreach (sort numerically keys %vartype2) { 
   ...
   open (INPUT_FILE, "<:raw", "$input_filename") or die "Can not open Raw Data File $input_filename\n";
   open (OUTPUT_FILE, ">:raw", "$direct_out/$output_filename") or die "Can not open Output File $direct_out/$output_filename\n";
   #
   # Read the header
   #
