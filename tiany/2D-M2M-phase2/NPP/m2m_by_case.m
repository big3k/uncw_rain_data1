...
% given a case, do m2m, save results, and return a cell array of performance metrics

function result = m2m_by_case(case_data) 

load_model6d

%case_data="/data1/youy/morph_microwave_grid_gprof/npp_result_final_step1/202211/case_0503/sate_select_case_063.mat";

case_data=string(case_data); 
load(case_data); 

[m2m, sys_prop]=func_m2m_case(sate_select, models, trans, md_idxes); 
if ismissing(m2m) 
   result=missing
else
...
geo_plot_2d_rain(sate_select.gprof_gmi_wz, sate_select.lat, sate_select.lon, "GMI", ...
                 strcat(new_dir, '/gmi.png'), crange=[0, 9]);
...
result = {new_dir, mat_name, perf, sys_prop}; 
